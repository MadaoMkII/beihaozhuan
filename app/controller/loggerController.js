    ...
    async getLogger(ctx) {
        try {
            ...
            if (!ctx.helper.isEmpty(condition.beginTime)) {
                let localTime = DateTime.fromISO(condition.beginTime);
                condition.date.$gte = localTime.plus({hour: -8}).toJSDate();
            }
            if (!ctx.helper.isEmpty(condition.endTime)) {
                let localTime = DateTime.fromISO(condition.endTime);
                condition.date.$lte = localTime.plus({hour: -8}).toJSDate();
            }

            delete condition.beginTime;
            delete condition.endTime;
            let count = await this.getFindModelCount(`Logger`, condition);
            let result = await ctx.model[`Logger`].find(condition, {_id: 0}, option);
