...
app.post('/getReport', async (req, res) => {
  ...
  const returnUrl = req.body.cookieStr;
  // returnUrl = `https://www.567pan.com/file-${returnUrl}.html`;
  const urlQuery = url.parse(returnUrl, true).pathname.replace('/file-', '').replace('.html', '');
  const queryObj = {
    type: '567new',
    fileId: urlQuery,
  ...
  const [ resBody ] = await requestMethod(queryObj, 'GET', 'http://www.qianxunsw.com/WebForm1.aspx');

  const realUrl = resBody.toString().split('<!DOCTYPE html>')[0].trim();
